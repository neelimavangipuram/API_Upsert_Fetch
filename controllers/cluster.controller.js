...
// we add the last date with the recurring peroid.
...
exports.apply_cluster = function (user_groups) {
    ...
    _.forEach(user_groups, function (company_groups) {
        _.forEach(company_groups, function (obj_arr) {
            ...
            docs = _.filter(obj_arr, function (o) {
                return o.days != 0
            });
            var mode = mode_value(_.map(docs, 'days'));
            if (mode > 0) {
                let all_txns = _.filter(docs, function (obj) {
                    return obj.days == mode
                });
                if (all_txns.length > 0) {
                    let last_txn = _.sortBy(all_txns, o => o.date)[all_txns.length - 1];
                    let output = {
                        name:last_txn.name,
                        user_id: last_txn.user_id,
                        next_amt: mean_value(_.map(all_txns, 'amount')), // Mean of all recurring txns will give next probable txn amount.
                        next_date: addDays(last_txn.date, mode), // Adding the mode value of days with the last txn date will give next txn date. 
                        transactions: all_txns
                    }
                    final_recurring_list.push(output);
                }
            }
        });
    });
    return _.sortBy(final_recurring_list, o => o.name);
}